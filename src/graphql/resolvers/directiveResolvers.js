...
module.exports = {
  isAuthenticated: (next, source, args, context,info) => {
    ...
    if (!token) {
      return new Error(`You must be authenticated to access "${info.fieldName}"`)
    }
    try {
      if(context.req.user){
        return next();
      ...
      return new Error(`Invalid token given`);
    } catch (err) {
    ...
    try {
      ...
      if(context.req.user){
       if (context.req.user.role.includes(args.role)) {
          ...
          return new Error(`You must be ${args.role} to access "${info.fieldName}"`);
        }
